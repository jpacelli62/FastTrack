...
namespace Faaast.Tests.Authentication
{
    public class OauthServerTests : IClassFixture<AuthFixture>
    {
        ...
        private async Task<Transaction> SendClientCredentialsAsync(string clientId, string clientSecret, string audience) => await SendPostAsync(this.Server, this.Fixture.DefaultOptions.TokenEndpoint, new Dictionary<string, string>
            {
        ...
        {
            var transaction = await this.SendClientCredentialsAsync("wrongid", this.Fixture.DefaultOptions.ClientSecret, Audience);
        ...
        {
            var transaction = await this.SendClientCredentialsAsync(this.Fixture.DefaultOptions.ClientId, "wrongpassword", Audience);
            Assert.Equal(HttpStatusCode.BadRequest, transaction.Response.StatusCode);
            Assert.Equal(Faaast.Authentication.OAuth2Server.Resources.Msg_InvalidClient, transaction.ResponseText);
        }
        ...
        {
            var transaction = await this.SendClientCredentialsAsync(this.Fixture.DefaultOptions.ClientId, this.Fixture.DefaultOptions.ClientSecret, "wrongaudience");
            Assert.Equal(HttpStatusCode.BadRequest, transaction.Response.StatusCode);
            Assert.Equal(Faaast.Authentication.OAuth2Server.Resources.Msg_InvalidClient, transaction.ResponseText);
        }
